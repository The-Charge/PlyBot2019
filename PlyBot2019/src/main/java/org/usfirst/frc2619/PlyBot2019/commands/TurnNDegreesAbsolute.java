...
public class TurnNDegreesAbsolute extends Command {
    ...
    private int flipCtr;
    private double currentAngleDegrees, motorSpeed, endThreshold;
    private boolean flip;   //true = clockwise, false = counterclockwise
    private AHRS gyro;

    // BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=CONSTRUCTOR
    public TurnNDegreesAbsolute(double targetAngleDegrees) {
    ...
    protected void initialize() {
        ...
        if(MathUtil.calcDirection(currentAngleDegrees, m_targetAngleDegrees) == 1)
            flip = true;
        else
            flip = false;
    }
    ...
    protected void execute() {
        currentAngleDegrees = gyro.getAngle(); //sets the currentAngle to the gyro's current angle
        
        SmartDashboard.putNumber("Gyro Angle: ", currentAngleDegrees);
    ...
    protected boolean isFinished() {
        ...
        if((Math.abs(currentAngleDegrees - m_targetAngleDegrees) <= endThreshold || 360 - Math.abs(currentAngleDegrees - m_targetAngleDegrees) <= endThreshold) && flipCtr > 10)    //flipCtr threshold can be changed
            return true;
